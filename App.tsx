...
const App: React.FC = () => {
  ...
  const applyAiResult = (result: any) => {
    if (result) {
      ...
      if (result.deliveryFee !== undefined) setDeliveryFee(result.deliveryFee);
  ...
  const handleAiAction = async () => {
    ...
    if (applyAiResult(result)) {
      ...
      alert("AI couldn't parse that. Please try adding more details.");
    }
  };
      ...
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          reader.onloadend = async () => {
            ...
            setIsAiLoading(false);
            if (!applyAiResult(result)) {
              alert("Could not extract details from voice note. Please speak clearly.");
            }
          };
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-bold text-slate-800">Smart Entry</h2>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Try voice</span>
                    <button 
                      onClick={toggleRecording}
                      className={`p-3 rounded-full transition-all ${isRecording ? 'bg-red-500 text-white recording-pulse' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                    >
              ...
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-8">
                <section className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500 text-slate-900 outline-none" placeholder="John Doe" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase">Phone Number</label>
                      <input type="tel" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500 text-slate-900 outline-none" placeholder="08012345678" />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase">Delivery Address</label>
                    <input type="text" value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500 text-slate-900 outline-none" placeholder="Street, City, State" />
                  </div>
                </section>
                ...
                <section className="space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                    ...
                    <button onClick={addItem} className="text-sm font-bold text-blue-600 flex items-center gap-1 hover:text-blue-700"><Plus size={16} /> Add Item</button>
                  </div>
                    ...
                    {items.map((item) => (
                      <div key={item.id} className="flex flex-col md:flex-row gap-3 items-end md:items-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                        <div className="flex-1 w-full space-y-1">
                          ...
                          <input type="text" value={item.description} onChange={(e) => updateItem(item.id!, 'description', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white border border-slate-200 text-slate-900 outline-none" placeholder="Dangote Cement 42.5R" />
                        </div>
                        ...
                        <div className="w-full md:w-40 space-y-1">
                          ...
                          <div className="relative">
                            ...
                            <input type="number" value={item.unitPrice} onChange={(e) => updateItem(item.id!, 'unitPrice', parseInt(e.target.value) || 0)} className="w-full pl-7 pr-3 py-2 rounded-lg bg-white border border-slate-200 text-slate-900 outline-none" />
                          </div>
                        </div>
                        <button onClick={() => removeItem(item.id!)} className="p-2 text-slate-300 hover:text-red-500"><Trash2 size={18} /></button>
                      </div>
                    ))}
                  </div>
                </section>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                       <div className="space-y-4">
                          <div className="flex items-center gap-2 border-b border-slate-100 pb-2"><Truck className="text-slate-400" size={18} /><h3 className="font-bold text-slate-700">Logistics</h3></div>
                          <div className="space-y-1">
                            ...
                            <div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">₦</span>
                              <input type="number" value={deliveryFee} onChange={(e) => setDeliveryFee(parseInt(e.target.value) || 0)} className="w-full pl-8 pr-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 outline-none" />
                            </div>
                          </div>
                       </div>
                       <div className="space-y-4">
                          <div className="flex items-center gap-2 border-b border-slate-100 pb-2"><Tag className="text-slate-400" size={18} /><h3 className="font-bold text-slate-700">Discount</h3></div>
                          <div className="flex items-center gap-2">
                            {[0, 1, 2, 3].map(pct => (
                              <button key={pct} onClick={() => setDiscountPercent(pct)} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all border ${discountPercent === pct ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
                                {pct === 0 ? 'None' : `${pct}%`}
                              </button>
                            ))}
                          </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 border-b border-slate-100 pb-2"><FileText className="text-slate-400" size={18} /><h3 className="font-bold text-slate-700">Notes</h3></div>
                      <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 min-h-[140px] outline-none" placeholder="Additional notes..." />
                    </div>
                  </div>
                </section>
              </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-200 sticky top-28">
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex justify-between text-slate-500"><span>Subtotal</span><span className="font-bold text-slate-800">{formatCurrency(totals.subtotal)}</span></div>
                  {discountPercent > 0 && <div className="flex justify-between text-green-600"><span>Discount ({discountPercent}%)</span><span className="font-bold">-{formatCurrency(totals.discountAmount)}</span></div>}
                  <div className="flex justify-between text-slate-500"><span>Delivery</span><span className="font-bold text-slate-800">{formatCurrency(deliveryFee)}</span></div>
                  <div className="pt-4 border-t border-slate-100 flex justify-between items-center"><span className="text-lg font-black text-slate-800 uppercase tracking-tighter">Total</span><span className="text-2xl font-black text-blue-600">{formatCurrency(totals.total)}</span></div>
                </div>
                <button onClick={handleCreateInvoice} disabled={!customerName || items.some(i => !i.description)} className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-4 rounded-xl font-black text-lg shadow-lg shadow-blue-200 transition-all flex items-center justify-center gap-2">
        ...
        {view === 'history' && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between"><h2 className="text-xl font-bold text-slate-800">History</h2></div>
            {invoices.length > 0 ? (
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {invoices.map((inv) => (
                      <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-4"><div className="font-mono text-sm font-bold text-slate-600">{inv.invoiceNumber}</div><div className="text-[10px] text-slate-400 uppercase font-bold">{new Date(inv.date).toLocaleDateString()}</div></td>
                        <td className="px-6 py-4"><div className="font-black text-slate-800 uppercase">{inv.customer.name}</div></td>
                        <td className="px-6 py-4"><span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${inv.status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{inv.status}</span></td>
        ...
        {view === 'preview' && currentInvoice && (
          <div className="max-w-4xl mx-auto">
            <div className="flex flex-wrap items-center justify-between mb-8 gap-4 no-print">
              <button onClick={() => setView('history')} className="text-slate-500 hover:text-slate-800 font-bold flex items-center gap-2"><ChevronRight size={18} className="rotate-180" /> Back</button>
              <div className="flex flex-wrap items-center gap-2">
                <button onClick={() => handleShareFile('image')} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl font-bold flex items-center gap-2 hover:bg-slate-200 transition-all shadow-sm"><ImageIcon size={18} /> Share Image</button>
                <button onClick={() => handleShareFile('pdf')} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl font-bold flex items-center gap-2 hover:bg-slate-200 transition-all shadow-sm"><FileDown size={18} /> Share PDF</button>
                <a href={generateWhatsAppLink(currentInvoice)} target="_blank" className="px-4 py-2 bg-[#25D366] text-white rounded-xl font-bold flex items-center gap-2 hover:opacity-90 shadow-sm transition-all"><MessageCircle size={18} /> Send Text</a>
              </div>
            </div>
            ...
            <div ref={invoiceRef} className="bg-white p-12 md:p-16 rounded-2xl shadow-xl border border-slate-200 relative overflow-hidden">
              <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-16 relative z-0">
                ...
                <div className="text-right flex flex-col items-end">
                  <h2 className="text-5xl font-black text-slate-200 mb-4 tracking-tighter uppercase">Invoice</h2>
                  <div className="space-y-2">
                    <div className="flex justify-end gap-4 text-sm"><span className="text-slate-400 font-bold uppercase tracking-wider">Number</span><span className="font-mono font-bold text-slate-800">{currentInvoice.invoiceNumber}</span></div>
                    <div className="flex justify-end gap-4 text-sm"><span className="text-slate-400 font-bold uppercase tracking-wider">Date</span><span className="font-bold text-slate-800">{new Date(currentInvoice.date).toLocaleDateString()}</span></div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-end mb-16">
                <div className="w-full md:w-80 space-y-4">
                  <div className="flex justify-between items-center text-slate-500 text-sm"><span className="font-bold uppercase tracking-widest">Subtotal</span><span className="font-bold">{formatCurrency(currentInvoice.subtotal)}</span></div>
                  <div className="pt-4 border-t-2 border-slate-900 flex justify-between items-center"><span className="font-black uppercase text-sm tracking-widest text-slate-900">Total</span><span className="text-3xl font-black text-blue-600">{formatCurrency(currentInvoice.totalAmount)}</span></div>
                </div>
              </div>
            </div>
          </div>
